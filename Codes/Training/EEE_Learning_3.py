...
from DataLoader2_txt import DataLoad2

from Unet1 import UNet
from Cnet1 import CNet

from loss import dice_coef, cut_crossentry_softmax, balance_bce_loss, cut_crossentry, stenosis_loss
...
#stage3        
        ...
        #loss31 = criterion1(target21, output21)
...
def train_net(net1, net2, n_epochs=200, batch_size=1, lr=1e-4, model1_name='Evaluation1_66_3.dat',
              model2_name='Evaluation2_66_3.dat'):
    shape = (224, 288, 288)
    ...
    # criterion = cut_crossentry()
    criterion1 = balance_bce_loss()
    criterion2 = balance_bce_loss()

    for epoch in range(n_epochs):
        train_epoch(net1, net2, train_dataloader1, train_dataloader2, optimizer1, optimizer2, criterion1, criterion2, epoch, n_epochs)
...
def predict(model, save_path, shape, img_path, num_classes):
    ...
    for t in range(file_num):
        ...
        shape1 = np.load('/F/SRTP/Npy/'+s.rstrip('.raw')+'.npy')
        #print(shape1)
        x = shape1[2]
        y = shape1[1]
        z = shape1[0]
        ...
        for i in range(z // stride_x - 1):
            for j in range(y // stride_y - 1):
                ...
                output = model(image_i)
                output = output.data.cpu().numpy()
                predict[:, :, i * stride_x:i*stride_x + shape[0], j * stride_y:j*stride_y+shape[1], x-shape[2]:x] += output * map_kernal
            ...
            for k in range(x // stride_z - 1):
                ...
                output = model(image_i)
                output = output.data.cpu().numpy()
                predict[:, :, i * stride_x:i*stride_x + shape[0], y - shape[1]:y, k * stride_z:k * stride_z + shape[2]] += output * map_kernal

                n_map[:, :, i * stride_x:i*stride_x + shape[0], y - shape[1]:y, k * stride_z:k * stride_z + shape[2]] += map_kernal
...
def Dice(label_dir, pred_dir):
    ...
    #f = open('DICE/DICE_'+str(pred_dir)+'.txt','w')
    #save_image = 'D:\Project\Result' 
    for t in range(file_num):
        image_path = file[t]
        s = image_path.split('/')
        s = str(s[-1])
        ...
        #predict = predict.astype(dtype=np.uint8)
        #predict.tofile((join(save_image, imagename)))
        print(s, dice[i])
        #f.write(imagename + ':' + str(dice[i]) + '\n')
        name.append(s)
        list1.append(dice[i])
        i+=1
    ...
    #f.close()
    #print(list1)
    #total = pd.DataFrame(data=list1)
    #total.to_csv('dice1.csv')
    return dice

if __name__ == '__main__':
    # net = DenseBiasNet(n_channels=1, n_classes=1)
    net1 = UNet(n_channels=1, n_classes=1)
    net2 = CNet(n_channels=1, n_classes=1)
    train_net(net1=net1, net2=net2, n_epochs=150, batch_size=1, lr=1e-4)